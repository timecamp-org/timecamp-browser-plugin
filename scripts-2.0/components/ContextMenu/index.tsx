...
const ContextMenu: React.FC<ContextMenuInterface> = (props) => {
  ...
  const [billableInputVisibility, setBillableInputVisibility] = useState<
    boolean | null
  >(props.billableInputVisibility);
  ...
  const [clearTriggerForTimePicker, setClearTriggerForTimePicker] =
  ...
  const [isBackendIntegration, setIsBackendIntegration] = useState(
    props.isBackendIntegration
  );
  const [noTaskFoundDisplayAlert, setNoTaskFoundDisplayAlert] =
    useState<boolean>(false);
  const [
    taskNotFoundInBackendIntegrationInfo,
    setTaskNotFoundInBackendIntegrationInfo,
  ...
  const [dontShowAdSettingValue, setDontShowAdSettingValue] =
    useState<number>(1);
  const [embedOnPopup, setEmbedOnPopup] = useState<boolean>(
    props.embedOnPopup !== undefined
  );
  const [trelloPowerUpAdVisible, setTrelloPowerUpAdVisible] = useState<boolean>(
    props.trelloPowerUpAdVisible ?? false
  );
  const [errorUnknownVisible, setErrorUnknownVisible] =
    useState<boolean>(false);
  const [errorUnknownMessage, setErrorUnknownMessage] = useState<string>("");
  const [errorMaintenanceModeVisible, setErrorMaintenanceModeVisible] =
    useState<boolean>(false);
  const [errorNoInternetVisible, setErrorNoInternetVisible] =
    useState<boolean>(false);
  const [errorSubscriptionExpiredVisible, setSubscriptionExpiredVisible] =
    useState<boolean>(false);
  const [durationFormat, setDurationFormat] = useState<number>(
    timeFormatter.DEFAULT_FORMAT
  );
  const [startTime, setStartTime] = useState<Date | null>(null);
  const [stopTime, setStopTime] = useState<Date | null>(null);

  const [wrongDatesErrorMessage, setWrongDatesErrorMessage] =
    useState<string>("");
  ...
  useEffect(() => {
    ...
    setTaskNotFoundInBackendIntegrationInfo(
      props.taskNotFoundInBackendIntegrationInfo
    );
  ...
  const getBackendIntegrationAdData = (userId: number) => {
    browser.runtime
      ...
      .then((resolve) => {
        if (
          resolve.modify_time === false ||
          new Date().getTime() - new Date(resolve.modify_time).getTime() >
            THIRTY_DAYS_IN_MILISEC
        ) {
  ...
  const setIsBackendIntegrationAndUserHasIntegration = (
  ...
  const getIntegrationAdMessage = () => {
    if (isAdmin) {
      return ReactHtmlParser(
        translate("backend_integration_ad_message_admin")
          .replace(
            "*service*",
            pathService.getIntegrationNameFromMessage(service)
          )
          .replace(
            "*linkStart*",
            "<a style='text-transform: capitalize; text-decoration: none; color: #2380e3;' target='_blank' href='" +
              pathService.getIntegrationUrl(service) +
              "'>"
          )
          .replace("*linkClose*", "</a>")
      );
    ...
    return ReactHtmlParser(
      translate("backend_integration_ad_message_user")
        .replace(
          "*service*",
          pathService.getIntegrationNameFromMessage(service)
        )
        .replace(
          "*linkStart*",
          "<a style='text-transform: capitalize; text-decoration: none; color: #2380e3;' target='_blank' href='" +
            pathService.getIntegrationMarketingWebsiteUrl(service) +
            "'>"
        )
        .replace("*linkClose*", "</a>")
    );
  ...
  const hasExtendedHeight =
    props.isBackendIntegration &&
    !isBackendIntegration &&
    dontShowAdSettingValue === 0;

  return (
    <div
      ref={node}
      className={`context-menu  ${!open ? "context-menu--hidden" : ""} ${
        hasExtendedHeight ? "context-menu-extended-height" : ""
      }`}
    ...
    >
      <div className={"context-menu-overflow-wrapper"}>
        ...
        <TaskPicker
          ...
          onNotFoundTaskForActiveBackendIntegration={
            onNotFoundTaskForActiveBackendIntegration
          }
          onAutoDetectTaskForActiveBackendIntegration={
            onAutoDetectTaskForActiveBackendIntegration
          }
          userId={userId}
        ...
        <TimeSelectors
          ...
          onStartTimeValueChange={(value) => {
            ...
            setWrongDatesErrorMessage(
              now < value && stopTime === null
                ? translate("Start time cannot be greater than now")
                : ""
            );
          }}
        ...
        />
        <WrongDatesError
          message={wrongDatesErrorMessage}
          visible={wrongDatesErrorMessage !== ""}
        />
